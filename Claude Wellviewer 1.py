...
if show_strat and strat_cfg:
    # Radio exterior de la columna estrat = 1.6x el casing mas externo
    # Se dibuja como anillos concéntricos a la derecha del pozo (eje Y positivo)
    r_strat_in  = R_REF_EXT * 1.9   # borde interno del bloque estrat
    r_strat_out = R_REF_EXT * 3.2   # borde externo del bloque estrat
    ...
    for si, s in enumerate(strat_cfg):
        ...
        # Bloque de formacion: sector angular en el lado derecho (Y>0)
        # Se usa un tubo completo (360°) semi-transparente a la derecha
        ang_s  = np.radians(360)
        nt_s   = NT_FULL
        th_s   = np.linspace(0, ang_s, nt_s, endpoint=False)

        x_out = r_strat_out * np.cos(th_s)
        y_out = r_strat_out * np.sin(th_s)
        x_in  = r_strat_in  * np.cos(th_s)
        y_in  = r_strat_in  * np.sin(th_s)

        x_v = np.concatenate([x_out, x_in, x_out, x_in])
        y_v = np.concatenate([y_out, y_in, y_out, y_in])
        z_v = np.concatenate([
            np.full(nt_s, z_top_s), np.full(nt_s, z_top_s),
            np.full(nt_s, z_bot_s), np.full(nt_s, z_bot_s),
        ])

        # Caras laterales externas e internas
        il_s, jl_s, kl_s = [], [], []
        n2 = nt_s; n3 = 2*nt_s; n4 = 3*nt_s
        for ii in range(nt_s):
            i0=ii; i1=(ii+1)%nt_s
            # cara externa (tope→fondo)
            il_s+=[i0,    i0   ]; jl_s+=[i1,    i0+n2]; kl_s+=[i0+n2, i1+n2]
            # cara interna (tope→fondo)
            il_s+=[i0+n2, i0+n2]; jl_s+=[i1+n2, i0+n3]; kl_s+=[i0+n3, i1+n3]
            # tapa superior
            il_s+=[i0,    i0   ]; jl_s+=[i1,    i0+nt_s]; kl_s+=[i0+nt_s, i1+nt_s]
            # tapa inferior
            il_s+=[i0+n2, i0+n2]; jl_s+=[i1+n2, i0+n3 ]; kl_s+=[i0+n3, i1+n3]

        nm_s = f"Estrat: {s['name']}"
        fig.add_trace(go.Mesh3d(
            x=x_v, y=y_v, z=z_v,
            i=il_s, j=jl_s, k=kl_s,
            color=s["color"], opacity=op_strat,
            name=nm_s, showlegend=(si == 0),
        ...
        # Linea horizontal de contacto en el tope
        if show_contact_lines:
            th_line = np.linspace(0, 2*np.pi, NT_FULL, endpoint=False)
            x_line  = r_strat_out * 1.05 * np.cos(th_line)
            y_line  = r_strat_out * 1.05 * np.sin(th_line)
            # Linea recta de tope — a lo largo del diámetro Y
            fig.add_trace(go.Scatter3d(
                x=[r_strat_in, r_strat_out],
                y=[0, 0],
                z=[z_top_s, z_top_s],
                mode="lines",
        ...
        if show_strat_labels:
            z_mid = (z_top_s + min(z_bot_s, Z1)) / 2
            fig.add_trace(go.Scatter3d(
                x=[r_strat_out * 1.08],
                y=[0],
                z=[z_mid],
